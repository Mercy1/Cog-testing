...
class KickBanMixin(MixinMeta):
    ...
    async def ban_user(
        ...
        if author == user:
            return _("As big-brain as it would be to ban yourself, I'm not going to let you. 🙏")
    ...
    async def kick(self, ctx: commands.Context, user: discord.Member, *, reason: str = None):
        ...
        if author == user:
            await ctx.send("Are you nuts? No, you can't kick yourself. Bad human, bad!")
            return
        elif not await is_allowed_by_hierarchy(self.bot, self.settings, guild, author, user):
            await ctx.send(
                _(
        ...
        try:
            ...
            try:
                ...
                await ctx.send(e) #See -> I'm F**king bored. - Dan
            kickmsglist = ["Done. Hah, what an idiot.", "User kicked, here's hoping they take the hint.","What a bloody idiot. At least they're gone.",
            "Don't hit your head on the way out! *I hope he does...*", "Done. I mean, not *done* but like, kicked. Ew. You're disgusting.",
            "Pool's closed, kid.","Done. That felt good.", "Congratulations! That user is gone. For now.", "Oh hey, what does this button d-",
            "Monkey see, Monkey doo, Monkey gets kicked from twycross zoo.", "https://media.giphy.com/media/lgcUUCXgC8mEo/giphy.gif"]
            kickmsg = random.choice(kickmsglist)
    ...
    async def ban(
        ...
        *,
        ...
        if result is True:
            banmsglist = ["User was lost in the pocket dimension." , "User Terminated successfully." , "Aaannnd it's gone. Forever." ,
            "This user fought the banhammer - and lost.", "This game of hide 'n' seek is over. Goodbye!" , "Stop, he's already dead!" , 
            "Well, so much for that guy."]
            banmsg = random.choice(banmsglist)
