...
class TempMutes(MixinMeta):
    ...
    def Config(self, bot):
        ...
        defaults = {"muted": {}}
        self.register_guild(**defaultsguild)
        self.register_global(**defaults)
    ...
    async def roleunmute(self, user, guildid, *, moderator: discord.Member = None):
        ...
        if guild is None:
            return
        mutedroleid = await self.guild(guild).muterole()
        ...
        async with self.muted() as muted:
    ...
    async def create_muted_role(self, guild):
        ...
        await self.guild(guild).muterole.set(muted_role.id)
    ...
    async def rolemute(
        ...
        *,
        ...
        guild = ctx.guild
        roleid = await self.guild(guild).muterole()
        if roleid is None:
            await ctx.send(
        ...
        async with self.muted() as muted:
            ...
            for user in users:
                ...
                if not await is_allowed_by_hierarchy(
                    self.bot, self, guild, ctx.author, user
                ):
                    failed.append(
    ...
    async def roleset(self, ctx, role: discord.Role):
        """Set a mute role."""
        await self.guild(ctx.guild).muterole.set(role.id)
